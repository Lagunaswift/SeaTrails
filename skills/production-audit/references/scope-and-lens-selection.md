...
- **`code-quality`**: any codebase review. Checks for magic numbers, loose equality, empty catches, naming inconsistencies, async anti-patterns, deferred implementations, and AI-generated code tells. Especially useful when assessing code that may have been AI-generated and merged without senior review.
- **`dependency-audit`**: any project with third-party dependencies. Reads lock files and package manifests for supply chain risks — unpinned versions, postinstall scripts, dependency bloat, missing CI scanning. Skip for zero-dependency libraries.
- **`infrastructure-config`**: any project with deployment configuration files in the repo (Dockerfiles, Terraform, K8s manifests, CI/CD pipelines, nginx configs). Checks for containers as root, open IAM, secrets in build args, unpinned base images. Skip for client-side-only projects with no deployment config.
